  ...
  const handleScanSuccess = async (decodedText: string) => {
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/api/company/bookings/verify`,
        { qrCode: decodedText },
        { headers: { Authorization: `Bearer ${token}` } },
      );
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600 mt-1">
            Verify and manage customer check-ins
          </p>
        </div>
        <button
          ...
          bookings.map((booking) => (
            ...
            >
              ...
              <p className="text-sm text-slate-500 mb-4">
                {booking.schedules.department.name}
              </p>
              <div className="flex items-center gap-2 text-sm font-medium text-slate-700 border-t pt-4">
                <CheckCircle size={16} className="text-green-500" />
                {new Date(booking.schedules.date).toLocaleDateString()} at{" "}
                {new Date(booking.schedules.startTime).toLocaleTimeString([], {
                  hour: "2-digit",
                  minute: "2-digit",
                })}
              </div>
        ...
        {showScanner && (
          ...
          >
            <div className="bg-white rounded-3xl p-8 max-w-lg w-full shadow-2xl relative">
              ...
              <h3 className="text-2xl font-extrabold text-slate-900 mb-6 pr-12">
                Verify Booking QR
              </h3>
      ...
      <AnimatePresence>
        {verificationResult && (
          ...
          >
            <div
              className={`px-8 py-4 rounded-2xl shadow-2xl border flex items-center gap-4 ${
